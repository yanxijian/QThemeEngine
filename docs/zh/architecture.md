...
**控件 Role**：`button.*` / `edit.*` / `check.*` / `tab.*` / `scroll.*` / `menu.*` / `toolbar.*` / `combo.*` / `spin.*` / `header.*`（含 `.hover` / `.pressed` / `.focus` / `.disabled` 等后缀）  
**度量 Role**：`button.radius` / `button.height` / `edit.padding` / `check.indicator` / `scroll.thickness` / `menu.itemHeight` / `combo.radius` / `spin.buttonWidth` / `header.height` 等。
...
| **M3** | Slider / Progress / GroupBox / ToolTip + DPI | padding/radius/height × dpiScale |
