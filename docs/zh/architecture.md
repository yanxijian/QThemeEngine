...
**控件 Role**：`button.*` / `edit.*` / `check.*` / `tab.*` / `scroll.*` / `menu.*` / `toolbar.*` / `combo.*` / `header.*`（含 `.hover` / `.pressed` / `.focus` / `.disabled` 等后缀）  
**度量 Role**：`button.radius` / `button.padding` / `check.indicator` / `scroll.thickness` / `menu.itemHeight` / `combo.radius` / `header.height` 等（不是独立的 `size.*` 组）。
