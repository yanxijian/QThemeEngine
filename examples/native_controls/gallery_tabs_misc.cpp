...
namespace gallery
{
	QWidget* tabOwnerDraw(QWidget* parent)
	{
		...
		layout->addWidget(new QLabel(QStringLiteral("Secondary path: owner-drawn widgets read the same ThemeStore via "
	...
	QWidget* tabCoverage(QWidget* parent)
	{
		...
		QString html = QStringLiteral("<h3>对齐仓库文档（Fluent Pack 架构）</h3>"
...
#endif
#if defined(QTE_HAS_PRINTSUPPORT)
		html += QStringLiteral("<p>已链接 PrintSupport（打印对话框 + QPrintPreviewWidget）。</p>");
#endif
