...
namespace gallery
{
	QWidget* pageButtons(QWidget* parent)
	{
...

		{
			...
			row->addWidget(tool);
			auto* link = new QCommandLinkButton(QStringLiteral("Command link"), QStringLiteral("Secondary description"), box);
			row->addWidget(link);
			row->addStretch(1);
...

		{
			...
			auto* buttons = new QDialogButtonBox(
				QDialogButtonBox::Ok | QDialogButtonBox::Cancel | QDialogButtonBox::Apply | QDialogButtonBox::Help, box);
	...
	QWidget* pageInput(QWidget* parent)
	{
		...
		auto* completer = new QCompleter(QStringList{QStringLiteral("alpha"), QStringLiteral("amber"), QStringLiteral("beta"),
	...
	QWidget* pageStates(QWidget* parent)
	{
		...
		layout->addWidget(new QLabel(QStringLiteral("Enabled vs Disabled / Default — Fluent state acceptance"), root));
	...
	QWidget* pageDisplay(QWidget* parent)
	{
		...
		browser->setHtml(QStringLiteral("<h3>QTextBrowser</h3><p>Read-only rich text with "
...

		{
			...
			auto* row = new QHBoxLayout;
			for (auto shape : {QFrame::Box, QFrame::Panel, QFrame::StyledPanel, QFrame::HLine, QFrame::VLine})
			{
