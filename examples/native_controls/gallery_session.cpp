...
namespace gallery
{
	void attachDockWidgets(QMainWindow* window)
	{
		...
		list->addItems({QStringLiteral("Dock item 1"), QStringLiteral("Dock item 2"), QStringLiteral("Dock item 3")});
	...
	bool verifyGallerySession(QApplication* app, QWidget* root, QString* report)
	{
		...
		else
		{
			...
			require("standalone QTabBar", root->findChild<QTabBar*>(QStringLiteral("gallery.standaloneTabBar")) != nullptr);
