...
namespace gallery
{
	QWidget* pageItemViews(QWidget* parent)
	{
		...
		list->addItems(
			{QStringLiteral("List item 1"), QStringLiteral("List item 2"), QStringLiteral("List item 3"), QStringLiteral("Disabled item")});
		...
		table->setHorizontalHeaderLabels({QStringLiteral("Col A"), QStringLiteral("Col B"), QStringLiteral("Col C")});
		for (int r = 0; r < 4; ++r)
		{
			for (int c = 0; c < 3; ++c)
			{
		...
		auto* listModel = new QStringListModel(
			QStringList{QStringLiteral("Model row 1"), QStringLiteral("Model row 2"), QStringLiteral("Model row 3")}, root);
		...
		tableModel->setHorizontalHeaderLabels({QStringLiteral("A"), QStringLiteral("B"), QStringLiteral("C")});
		for (int r = 0; r < 3; ++r)
		{
			for (int c = 0; c < 3; ++c)
			{
	...
	QWidget* pageDialogs(QWidget* parent)
	{
		auto* root = new QWidget;
		auto* layout = new QVBoxLayout(root);
		layout->addWidget(new QLabel(QStringLiteral("Open stock dialogs to preview modal chrome (palette / buttons)."), root));
		...
		place(addBtn(QStringLiteral("MessageBox"),
					 [root]
					 {
						 QMessageBox::information(root, QStringLiteral("Info"), QStringLiteral("Sample information message."));
					 }));
		place(addBtn(QStringLiteral("Question"),
					 [root]
					 {
						 QMessageBox::question(root, QStringLiteral("Confirm"), QStringLiteral("Proceed with sample action?"));
					 }));
		place(addBtn(QStringLiteral("Warning"),
					 [root]
					 {
						 QMessageBox::warning(root, QStringLiteral("Warning"), QStringLiteral("Sample warning."));
					 }));
		place(addBtn(QStringLiteral("Critical"),
					 [root]
					 {
						 QMessageBox::critical(root, QStringLiteral("Error"), QStringLiteral("Sample critical error."));
					 }));
		...
		place(addBtn(QStringLiteral("ProgressDialog"),
					 [root]
					 {
						 QProgressDialog dlg(QStringLiteral("Working…"), QStringLiteral("Cancel"), 0, 100, root);
