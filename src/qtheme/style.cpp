...
namespace qtheme
{
	namespace
	{
		...
		void drawRounded(QPainter* painter, const QRect& rect, int radius, const QColor& fill, const QColor& border,
		...
		void drawFluentFocusRing(QPainter* painter, const QRect& rect, int radius, const QColor& outer, const QColor& inner,
								 qreal outerWidth, qreal innerWidth)
		{
	...
	void QThemeStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
	{
		...
		if (element == PE_FrameLineEdit || element == PE_PanelLineEdit)
		{
			...
			const QColor bg =
				roleColor(QStringLiteral("edit"), (option->state & State_Enabled) ? QStringLiteral("bg") : QStringLiteral("bg.disabled"),
						  option->palette.color(QPalette::Base));
			const QColor border = roleColor(QStringLiteral("edit"), borderRole, option->palette.color(QPalette::Mid));
		...
		if (element == PE_IndicatorCheckBox || element == PE_IndicatorRadioButton)
		{
			...
			QColor bg = roleColor(QStringLiteral("check"), QStringLiteral("bg"), option->palette.color(QPalette::Base));
			QColor border = roleColor(QStringLiteral("check"), QStringLiteral("border"), option->palette.color(QPalette::Mid));
			if (!enabled)
			{
				if (on || partial)
				{
			...
			{
				bg = roleColor(QStringLiteral("check"), hover ? QStringLiteral("bg.checked.hover") : QStringLiteral("bg.checked"), bg);
			...
			const int radius = radio ? option->rect.width() / 2 : roleMetric(QStringLiteral("check"), QStringLiteral("radius"), 3);
			...
			if (on || partial)
			{
				...
				painter->setRenderHint(QPainter::Antialiasing, true);
				const QColor ind =
					roleColor(QStringLiteral("check"), enabled ? QStringLiteral("indicator") : QStringLiteral("indicator.disabled"),
		...
		if (element == PE_PanelMenu || element == PE_FrameMenu)
		{
			...
			const QColor solid = roleColor(QStringLiteral("menu"), QStringLiteral("bg"), option->palette.color(QPalette::Window));
			const QColor bg = roleColor(QStringLiteral("menu"), QStringLiteral("bg.acrylic"), solid);
			const QColor border = roleColor(QStringLiteral("menu"), QStringLiteral("border"), option->palette.color(QPalette::Mid));
			if (radius > 0)
			{
		...
		if (element == PE_FrameFocusRect)
		{
			...
			if (!ambient.isValid())
			{
				ambient = roleColor(QStringLiteral("palette"), QStringLiteral("canvas"), option->palette.color(QPalette::Window));
			}
			QColor outer;
			QColor inner;
			...
			const qreal outerW = checkLike ? qreal(roleMetric(QStringLiteral("check"), QStringLiteral("focusWidth"), 1))
										   : qreal(roleMetric(QStringLiteral("focus"), QStringLiteral("outer"), 2));
			const qreal innerW = checkLike ? 0.0 : qreal(roleMetric(QStringLiteral("focus"), QStringLiteral("inner"), 1));
			drawFluentFocusRing(painter, option->rect, radius, outer, inner, outerW, innerW);
			return;
		}

		if (element == PE_IndicatorArrowDown || element == PE_IndicatorArrowUp || element == PE_IndicatorArrowLeft
			|| element == PE_IndicatorArrowRight || element == PE_IndicatorSpinUp || element == PE_IndicatorSpinDown
			|| element == PE_IndicatorSpinPlus || element == PE_IndicatorSpinMinus)
		{
			Qt::ArrowType type = Qt::DownArrow;
			if (element == PE_IndicatorArrowUp || element == PE_IndicatorSpinUp || element == PE_IndicatorSpinPlus)
			{
				type = Qt::UpArrow;
			}
			...
			const bool enabled = option->state & State_Enabled;
			const bool fromSpin = element == PE_IndicatorSpinUp || element == PE_IndicatorSpinDown || element == PE_IndicatorSpinPlus
								  || element == PE_IndicatorSpinMinus;
			const QString group = fromSpin ? QStringLiteral("spin") : QStringLiteral("combo");
			const QColor color = roleColor(group, enabled ? QStringLiteral("arrow") : QStringLiteral("arrow.disabled"),
		...
		if (element == PE_PanelButtonTool)
		{
			...
			const QColor bg = roleColor(QStringLiteral("button"), bgRole, option->palette.color(QPalette::Button));
			const QColor border = roleColor(QStringLiteral("button"), QStringLiteral("border"), option->palette.mid().color());
		...
		{
			const bool enabled = option->state & State_Enabled;
			const QColor bg = roleColor(QStringLiteral("groupbox"), QStringLiteral("bg"), option->palette.color(QPalette::Window));
			const QColor border = roleColor(QStringLiteral("groupbox"), QStringLiteral("border"), option->palette.mid().color());
		...
		{
			const QColor bg = roleColor(QStringLiteral("tooltip"), QStringLiteral("bg"), option->palette.color(QPalette::ToolTipBase));
			const QColor border = roleColor(QStringLiteral("tooltip"), QStringLiteral("border"), option->palette.mid().color());
		...
		{
			const QColor sep = roleColor(QStringLiteral("toolbar"), QStringLiteral("separator"), option->palette.mid().color());
			painter->save();
			painter->setPen(sep);
		...
		{
			const QColor handle = roleColor(QStringLiteral("toolbar"), QStringLiteral("handle"), option->palette.mid().color());
			painter->save();
		...
		if (element == PE_Frame || element == PE_FrameWindow)
		{
			...
			if (textEdit || (scroll && scroll->frameWidth() > 0 && !widget->inherits("QAbstractItemView")))
			{
				...
				const QColor bg =
					roleColor(group, enabled ? QStringLiteral("bg") : QStringLiteral("bg.disabled"), option->palette.color(QPalette::Base));
			...
			{
				const QColor bg = roleColor(QStringLiteral("calendar"), QStringLiteral("bg"), option->palette.color(QPalette::Base));
				const QColor border = roleColor(QStringLiteral("calendar"), QStringLiteral("border"), option->palette.mid().color());
			...
			{
				const QColor line = roleColor(QStringLiteral("frame"), QStringLiteral("line"), option->palette.mid().color());
				painter->save();
			...
			{
				const QColor bg = roleColor(QStringLiteral("frame"), QStringLiteral("bg"), option->palette.color(QPalette::Window));
				const QColor border = roleColor(QStringLiteral("frame"), QStringLiteral("border"), option->palette.mid().color());
		...
		{
			const QColor bg = roleColor(QStringLiteral("dock"), QStringLiteral("bg"), option->palette.color(QPalette::Window));
			const QColor border = roleColor(QStringLiteral("dock"), QStringLiteral("border"), option->palette.mid().color());
			painter->fillRect(option->rect, bg);
			painter->setPen(border);
			painter->drawRect(option->rect.adjusted(0, 0, -1, -1));
			return;
		...
		{
			const QColor bg = roleColor(QStringLiteral("status"), QStringLiteral("bg"), option->palette.color(QPalette::Window));
			painter->fillRect(option->rect, bg);
			if (element == PE_PanelStatusBar)
			{
				const QColor border = roleColor(QStringLiteral("status"), QStringLiteral("border"), option->palette.mid().color());
				painter->setPen(border);
		...
		{
			const QColor handle = roleColor(QStringLiteral("dock"), QStringLiteral("border"), option->palette.mid().color());
		...
		if (element == PE_IndicatorBranch)
		{
			...
			const QColor branch = roleColor(QStringLiteral("view"), QStringLiteral("branch"), option->palette.color(QPalette::WindowText));
	...
	void QThemeStyle::drawControl(ControlElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
	{
		if (element == CE_PushButtonBevel)
		{
			...
			if (btn && painter)
			{
				...
				const QColor bg = roleColor(QStringLiteral("button"), bgRole, btn->palette.color(QPalette::Button));
				...
				const QColor border =
					roleColor(QStringLiteral("button"), isDefault ? QStringLiteral("border.focus") : QStringLiteral("border"),
		...
		if (element == CE_PushButtonLabel)
		{
			...
			if (btn && painter)
			{
				...
				const QColor fg = roleColor(QStringLiteral("button"), fgRole, btn->palette.color(QPalette::ButtonText));
		...
		if (element == CE_TabBarTabShape)
		{
			...
			if (tab && painter)
			{
				...
				const QColor bg = roleColor(QStringLiteral("tab"), bgRole, tab->palette.color(QPalette::Window));
				const QColor border = roleColor(QStringLiteral("tab"), QStringLiteral("border"), tab->palette.mid().color());
				...
				{
					const QColor ind =
						roleColor(QStringLiteral("tab"), QStringLiteral("indicator"), tab->palette.color(QPalette::Highlight));
					painter->save();
					painter->setPen(Qt::NoPen);
					painter->setBrush(ind);
		...
		if (element == CE_TabBarTabLabel)
		{
			...
			if (tab && painter)
			{
				...
				const QColor fg = roleColor(QStringLiteral("tab"), fgRole, tab->palette.color(QPalette::WindowText));
		...
		if (element == CE_MenuItem)
		{
			...
			if (mi && painter)
			{
				...
				{
					const QColor sep = roleColor(QStringLiteral("menu"), QStringLiteral("separator"), mi->palette.mid().color());
				...
				const QColor panelBg = roleColor(QStringLiteral("menu"), QStringLiteral("bg"), mi->palette.color(QPalette::Window));
				const QColor hoverBg = roleColor(QStringLiteral("menu"), QStringLiteral("bg.hover"), panelBg);
				const QColor pressedBg = roleColor(QStringLiteral("menu"), QStringLiteral("bg.pressed"), hoverBg);
				...
				{
					const int inset = roleMetric(QStringLiteral("menu"), QStringLiteral("itemInset"), 2);
					const int itemRadius = roleMetric(QStringLiteral("menu"), QStringLiteral("itemRadius"), 4);
				...
				const QColor fg = roleColor(QStringLiteral("menu"), fgRole, mi->palette.color(QPalette::WindowText));
		...
		if (element == CE_MenuBarItem)
		{
			...
			if (mi && painter)
			{
				...
				const QColor bg = roleColor(QStringLiteral("menu"), bgRole, mi->palette.color(QPalette::Window));
				const QColor fg = roleColor(QStringLiteral("menu"), enabled ? QStringLiteral("bar.fg") : QStringLiteral("fg.disabled"),
		...
		{
			const QColor bg = roleColor(QStringLiteral("menu"), QStringLiteral("bar.bg"), option->palette.color(QPalette::Window));
			painter->fillRect(option->rect, bg);
			return;
		}
		...
		{
			const QColor bg = roleColor(QStringLiteral("toolbar"), QStringLiteral("bg"), option->palette.color(QPalette::Window));
			const QColor border = roleColor(QStringLiteral("toolbar"), QStringLiteral("border"), option->palette.mid().color());
			painter->fillRect(option->rect, bg);
			painter->setPen(border);
		...
		{
			const bool hover = option->state & State_MouseOver;
			const QColor bg = roleColor(QStringLiteral("splitter"), QStringLiteral("bg"), option->palette.window().color());
			const QColor handle = roleColor(QStringLiteral("splitter"), hover ? QStringLiteral("handle.hover") : QStringLiteral("handle"),
		...
		if (element == CE_DockWidgetTitle)
		{
			...
			if (dock && painter)
			{
				...
				const QColor bg = roleColor(QStringLiteral("dock"), active ? QStringLiteral("title.bg.active") : QStringLiteral("title.bg"),
											dock->palette.color(QPalette::Window));
				const QColor fg =
					roleColor(QStringLiteral("dock"), enabled ? QStringLiteral("title.fg") : QStringLiteral("title.fg.disabled"),
		...
		{
			const auto* btn = qstyleoption_cast<const QStyleOptionButton*>(option);
			if (btn && painter && (btn->features & QStyleOptionButton::CommandLinkButton))
			{
				const bool enabled = btn->state & State_Enabled;
				const QColor title =
					roleColor(QStringLiteral("commandlink"), enabled ? QStringLiteral("fg") : QStringLiteral("fg.disabled"),
							  btn->palette.color(QPalette::ButtonText));
				const QColor descFallback =
					roleColor(QStringLiteral("palette"), QStringLiteral("text.tertiary"), btn->palette.color(QPalette::WindowText));
				const QColor desc = roleColor(QStringLiteral("commandlink"),
		...
		if (element == CE_HeaderSection)
		{
			...
			if (header && painter)
			{
				...
				const QColor bg = roleColor(QStringLiteral("header"), bgRole, header->palette.color(QPalette::Button));
				const QColor border = roleColor(QStringLiteral("header"), QStringLiteral("border"), header->palette.mid().color());
		...
		if (element == CE_HeaderLabel)
		{
			...
			{
				const bool enabled = header->state & State_Enabled;
				const QColor fg = roleColor(QStringLiteral("header"), enabled ? QStringLiteral("fg") : QStringLiteral("fg.disabled"),
		...
		if (element == CE_ComboBoxLabel)
		{
			...
			if (combo && painter)
			{
				...
				const QColor fg = roleColor(QStringLiteral("combo"), enabled ? QStringLiteral("fg") : QStringLiteral("fg.disabled"),
		...
		if (element == CE_ProgressBarGroove)
		{
			const bool enabled = option->state & State_Enabled;
			const QColor groove =
				roleColor(QStringLiteral("progress"), enabled ? QStringLiteral("groove") : QStringLiteral("groove.disabled"),
						  option->palette.mid().color());
			const QColor border = roleColor(QStringLiteral("progress"), QStringLiteral("border"), option->palette.mid().color());
		...
		if (element == CE_ProgressBarContents)
		{
			...
			if (prog && painter)
			{
				...
				const QColor fill = roleColor(QStringLiteral("progress"), chunkRole, prog->palette.color(QPalette::Highlight));
				const int radius = roleMetric(QStringLiteral("progress"), QStringLiteral("radius"), 3);
		...
		if (element == CE_ProgressBarLabel)
		{
			...
			{
				const bool enabled = prog->state & State_Enabled;
				const QColor fg = roleColor(QStringLiteral("progress"), enabled ? QStringLiteral("fg") : QStringLiteral("fg.disabled"),
		...
		if (element == CE_ItemViewItem)
		{
			...
			if (item && painter)
			{
				...
				const QColor fg = roleColor(QStringLiteral("view"), fgRole, item->palette.color(QPalette::Text));
				const QColor selBg =
					roleColor(QStringLiteral("view"), active ? QStringLiteral("bg.selected") : QStringLiteral("bg.selected.inactive"),
							  item->palette.color(QPalette::Highlight));
				const QColor base = roleColor(QStringLiteral("view"), QStringLiteral("bg"), item->palette.color(QPalette::Base));
				const QColor alt =
					roleColor(QStringLiteral("view"), QStringLiteral("bg.alternate"), item->palette.color(QPalette::AlternateBase));
										 ...
										 const QWidget* widget) const
	{
		if (control == CC_ScrollBar)
		{
			...
			if (sb && painter)
			{
				...
				const QColor groove = roleColor(QStringLiteral("scroll"), QStringLiteral("groove"), sb->palette.window().color());
				...
				const QColor hc = roleColor(QStringLiteral("scroll"), handleRole, sb->palette.mid().color());
				...
				const QColor arrow =
					roleColor(QStringLiteral("scroll"), enabled ? QStringLiteral("arrow") : QStringLiteral("arrow.disabled"),
		...
		if (control == CC_Slider)
		{
			...
			if (slider && painter)
			{
				...
				const QColor grooveColor =
					roleColor(QStringLiteral("slider"), enabled ? QStringLiteral("groove") : QStringLiteral("groove.disabled"),
				...
				const QColor fillColor = roleColor(QStringLiteral("slider"), fillRole, slider->palette.color(QPalette::Highlight));
				...
				const QColor handleBg = roleColor(QStringLiteral("slider"), handleRole, slider->palette.button().color());
				const bool focused = (slider->state & State_HasFocus) && enabled;
				const QColor handleBorder =
					roleColor(QStringLiteral("slider"), focused ? QStringLiteral("handle.border.focus") : QStringLiteral("handle.border"),
							  slider->palette.mid().color());
		...
		if (control == CC_ComboBox)
		{
			...
			if (combo && painter)
			{
				...
				const QColor bg = roleColor(QStringLiteral("combo"), bgRole, combo->palette.color(QPalette::Button));
				const QColor border = roleColor(QStringLiteral("combo"), borderRole, combo->palette.mid().color());
				...
				const QColor arrow =
					roleColor(QStringLiteral("combo"), enabled ? QStringLiteral("arrow") : QStringLiteral("arrow.disabled"),
		...
		if (control == CC_SpinBox)
		{
			...
			if (spin && painter)
			{
				...
				const QColor bg = roleColor(QStringLiteral("spin"), enabled ? QStringLiteral("bg") : QStringLiteral("bg.disabled"),
											spin->palette.color(QPalette::Base));
				const QColor border = roleColor(QStringLiteral("spin"), borderRole, spin->palette.mid().color());
				...
				auto paintButton = [&](SubControl sc, bool up)
				{
					...
					const QColor btnBg = roleColor(QStringLiteral("spin"), btnRole, spin->palette.button().color());
					painter->fillRect(br.adjusted(1, 1, -1, -1), btnBg);
					const QColor arrow =
						roleColor(QStringLiteral("spin"), enabled ? QStringLiteral("arrow") : QStringLiteral("arrow.disabled"),
		...
		if (control == CC_ToolButton)
		{
			...
			if (tb && painter)
			{
				...
				if (tb->subControls & SC_ToolButtonMenu)
				{
					...
					const QColor arrow = roleColor(QStringLiteral("combo"),
												   (tb->state & State_Enabled) ? QStringLiteral("arrow") : QStringLiteral("arrow.disabled"),
		...
		if (control == CC_Dial)
		{
			...
			if (dial && painter)
			{
				...
				const QColor groove =
					roleColor(QStringLiteral("dial"), enabled ? QStringLiteral("groove") : QStringLiteral("groove.disabled"),
							  dial->palette.mid().color());
				const QColor fill = roleColor(QStringLiteral("dial"), enabled ? QStringLiteral("fill") : QStringLiteral("fill.disabled"),
											  dial->palette.color(QPalette::Highlight));
				const QColor handleBg = roleColor(QStringLiteral("dial"), QStringLiteral("handle"), dial->palette.button().color());
				const QColor handleBorder =
					roleColor(QStringLiteral("dial"), focused ? QStringLiteral("handle.border.focus") : QStringLiteral("handle.border"),
							  dial->palette.mid().color());
