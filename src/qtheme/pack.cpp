...
namespace qtheme
{
	...
	int PackRegistry::registerPacksInDirectory(const QString& dir, Error* err)
	{
		...
		const QStringList files = d.entryList(QStringList{QStringLiteral("*.theme.json")}, QDir::Files, QDir::Name);
	...
	bool PackRegistry::registerBuiltinFluentPacks(Error* err)
	{
		...
		const bool haveCore = hasPack(QString::fromUtf8(kPackFluentLight)) && hasPack(QString::fromUtf8(kPackFluentDark))
